...
fn analyze_project_counts_files_loc_and_langs() {
    ...
    let map = walk::analyze_project(&t.0, 10, 80);
    ...
    // 3 code files (package.json doesn't match JS ext list? json IS config)
    ...
    assert_eq!(map.total_loc, 7); // a.js 2 + b.py 3 + c.ts 1 + package.json 1 (config)
...
fn top_n_limits_key_files() {
    ...
    let map = walk::analyze_project(&t.0, 3, 80);
...
fn tree_lines_compacts_structure() {
    ...
    let map = walk::analyze_project(&t.0, 10, 80);
    let lines = walk::tree_lines(&map.dir_stats, 2, 40);
