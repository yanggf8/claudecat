...
fn package_json_full_parse() {
    ...
    common::write(&t.0, "package.json", r#"{"name":"api","main":"src/server.js","bin":{"cc":"src/cli.js"},"scripts":{"start":"node src/server.js","build":"tsc"},"dependencies":{"express":"^4","zod":"^3"},"devDependencies":{"typescript":"^5"}}"#);
    let (meta, deps) = manifest::detect_project_meta(&t.0);
    assert_eq!(meta.name, "api");
    assert_eq!(meta.language, "TypeScript/JavaScript");
    assert!(meta.entry_points.iter().any(|e| e.contains("src/server.js")));
    assert!(meta.entry_points.iter().any(|e| e.contains("src/cli.js")));
    assert!(meta.framework.contains("Express.js"));
    assert_eq!(meta.run_command.as_deref(), Some("npm start (node src/server.js)"));
...
fn pyproject_and_requirements() {
    ...
    common::write(&t.0, "requirements.txt", "requests==2.0\n# comment\nflask\n");
    let (meta, deps) = manifest::detect_project_meta(&t.0);
